...
● **AUTHOR :** [Tharuk Renuja](https://t.me/TharukRenuja) 
● **SERVER :** `Heroku` 
● **LIBRARY :** `Pyrogram` 
● **LANGUAGE :** `Python 3.9`
"""
...
CHANNEL_BUTTON = InlineKeyboardMarkup(
        [[
        InlineKeyboardButton('↗ Join Here ↗', url='https://t.me/SLBotsOfficial')
        ]]
    )
ADDME_BUTTON = InlineKeyboardMarkup(
        [[
        InlineKeyboardButton('↗ ADD ME TO A GROUP ↗', url="t.me/MemberStickerslBot?startgroup=true")
        ]]
    )
...
@SLBots.on_callback_query() # callbackQuery()
...
@SLBots.on_message(filters.command(["start"]) & filters.private)
...
@SLBots.on_message((filters.command(["start"]) & filters.group) | filters.regex("/start@MemberStickerslBot"))
...
@SLBots.on_message(filters.command(["ping"]))
...
@SLBots.on_message(filters.new_chat_members & filters.group)
async def sticker_group(bot, message):
   try:
      chat_id = int(message.chat.id)
      count = await SLBots.get_chat_members_count(chat_id)
      if count == 3:
...
@SLBots.on_message(filters.channel & filters.command(["start"]))
...
@SLBots.on_message(filters.command(["help"]))
